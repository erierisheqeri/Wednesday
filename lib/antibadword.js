...
async function handleAntiBadwordCommand(sock, chatId, message, match) {
    ...
    if (match === 'on') {
        ...
        if (existingConfig?.enabled) {
            return sock.sendMessage(chatId, { text: '*AntiBadword è già attivato per questo gruppo*' });
        }
        await setAntiBadword(chatId, 'on', 'delete');
        return sock.sendMessage(chatId, { text: '*AntiBadword è stato attivato. Usa .antibadword set <azione> per personalizzare l azione*' });
    }

    if (match === 'off') {
        const config = await getAntiBadword(chatId, 'on');
        if (!config?.enabled) {
            return sock.sendMessage(chatId, { text: '*AntiBadword è già disattivato per questo gruppo*' });
        }
        await removeAntiBadword(chatId);
        return sock.sendMessage(chatId, { text: '*AntiBadword è stato disattivato per questo gruppo*' });
    }

    if (match.startsWith('set')) {
        const action = match.split(' ')[1];
        if (!action || !['delete', 'kick', 'warn'].includes(action)) {
            return sock.sendMessage(chatId, { text: '*Azione non valida. Scegli: delete , kick, o warn*' });
        }
        await setAntiBadword(chatId, 'on', action);
        return sock.sendMessage(chatId, { text: `*AntiBadword action set to: ${action}*` });
    }

    return sock.sendMessage(chatId, { text: '*Comando non valido. Usa .antibadword per vedere come usarlo*' });
}
...
async function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
    ...
    if (config.words?.some(word => userMessage.toLowerCase().includes(word.toLowerCase()))) {
        ...
        // List of bad words
        const badWords = [
            'serbia', 'serbi', 'eri è gay', 'bsdk', 'fucker', 'bhosda', 
        ...
        switch (antiBadwordConfig.action) {
            case 'delete':
                await sock.sendMessage(chatId, {
                    text: `*@${senderId.split('@')[0]} NO COMUNISTI ❌🏳️‍🌈*`,
                    mentions: [senderId]
                });
                break;
